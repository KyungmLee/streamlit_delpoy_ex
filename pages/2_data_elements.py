...
DATA_PATH = Path(__file__).parent / 'data' / 'kor_news_250422.xlsx'
path = f'{Path(__file__).parent}'.split(r'\pages')
DATA_PATH = Path(path[0]) / 'data' / 'kor_news_250422.xlsx'
st.write(Path.home())
...
st.table(df.iloc[:5])
# table = st.table(df.iloc[:5])
# table.add_rows(df.iloc[12:20])